...
describe('NavBar', () => {
  ...
  beforeEach(() => {
    ...
    mockAuthStore = {
      get isReady() {
        return isReadyRef.value;
      },
      set isReady(val) {
        isReadyRef.value = val;
      },
      _isReady: isReadyRef,
      get isAuthenticated() {
    ...
    return mount(NavBar, {
      global: {
        ...
        stubs: {
          Menubar: {
            template:
              '<div class="menubar"><slot name="start" /><slot name="end" /><div v-for="item in model" :key="item.label" @click="item.command && item.command()">{{ item.label }}</div></div>',
            props: ['model'],
          },
          Button: {
