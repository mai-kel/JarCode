<template>
  <div class="w-full">
    <div class="p-fluid grid">
      <div class="col-12 lg:col-5">
        <Card>
          ...
          <template #content>
            ...
            <div v-else ref="listContainer" class="left-panel-scroll">
              <ul class="list-none p-0 m-0">
                <li
                  v-for="s in submissions"
                  :key="s.id"
                  class="p-2 border-bottom cursor-pointer"
                  :class="{ 'bg-100': selected && selected.id === s.id }"
                  @click="$emit('select', s)"
                >
                  <div>
                    ...
                    <div class="mt-2">
                      ...
                      <template v-else>
                        <span
                          :class="
                            outcomeInfo(s.result?.outcome).isPass ? 'outcome-pass' : 'outcome-fail'
                          "
                          :title="outcomeInfo(s.result?.outcome).label"
                        >
                          {{ outcomeInfo(s.result?.outcome).icon }}
                        </span>
                        <span class="text-sm text-color-secondary" style="margin-left: 8px">{{
                          outcomeInfo(s.result?.outcome).label
                        }}</span>
                      </template>
                    </div>
                  </div>
              ...
              <div v-if="submissions.length === 0" class="p-3 text-color-secondary">
                No submissions yet.
              </div>
      ...
      <div
        v-if="selected && selected.result && selected.result.ai_evaluation"
      ...
      >
        <Card>
          ...
          <template #content>
            <div class="submission-ai-evaluation" v-html="sanitizedAiEvaluation"></div>
          </template>
        </Card>
      </div>
    ...
    autoFillInterval = setInterval(() => {
      try {
        if (!props.hasNext || props.loading || props.loadingMore) {
          if (!props.hasNext && autoFillInterval) {
        ...
        console.error('auto-load loop error', e);
      }
    }, 250);
  } catch (e) {
    console.error('auto-load start error', e);
  }
}
</script>
